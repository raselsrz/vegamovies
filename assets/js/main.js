...
$(document).ready(function(){
	...
	$('.sorter').each(function(){
		...
		a.append('<span>'+b+'<span class="fa fa-angle-double-down" style="font-size: 10px;margin-left:8px"></span></span>');
		a.click(function() {
			$(this).find('form').fadeToggle(100);
			$(this).toggleClass('sort-act');
		});	
	});
...
const downloadLinksDiv = document.querySelector('.download-links-div');
if(downloadLinksDiv){
const downloadLinks = downloadLinksDiv.querySelectorAll('a');
...
const tabBox_ul = document.querySelector('.tab-box');
const allI_tags = document.querySelectorAll('.icon i');
...
all_tabs.forEach(function(tab){
    tab.addEventListener('click',function(){ 
        tabBox_ul.querySelector('.active').classList.remove('active')
        tab.classList.add('active')
    })
})*/

function GotoLink(link){
    var fullLink = 'https://' + window.location.host + '/' + link;
...
function handleIcon(){
    var scrollVal = Math.ceil(tabBox_ul.scrollLeft);
    var maxScrollableWidth = tabBox_ul.scrollWidth - tabBox_ul.clientWidth
    if(scrollVal > 2){
        $('.icon--left').css('display','flex');
        
    }else {
       $('.icon--left').hide();
    }
    if(maxScrollableWidth > (scrollVal + 10)){
        $('.icon--right').show();
...
allI_tags.forEach(function(iconI){
    iconI.addEventListener('click',function(){ 
        tabBox_ul.scrollLeft += iconI.id === "left" ? -350 : 350
...
const dragging = (e) => {
    ...
    tabBox_ul.scrollLeft -= e.movementX;
    tabBox_ul.classList.add('dragging')
...
const dragStop = () => {
    isDragging = false;
    tabBox_ul.classList.remove('dragging')
};

tabBox_ul.addEventListener('mousemove', dragging);
tabBox_ul.addEventListener('mousedown', () => isDragging = true);
tabBox_ul.addEventListener('mouseup', dragStop);
tabBox_ul.addEventListener('mouseleave', dragStop); // Add this line